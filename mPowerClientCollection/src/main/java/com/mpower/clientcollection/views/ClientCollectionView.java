...
import com.mpower.clientcollection.widgets.IBinaryWidget;
 ...
 * @author ratna halder
 */
public class ClientCollectionView extends ScrollView implements OnLongClickListener {

	// starter random number for view IDs
    private final static int VIEW_ID = 12345;  
    
    private final static String t = "mintelView";
    ...
    public ClientCollectionView(Context context, FormEntryPrompt[] questionPrompts,
        ...
        // added or modified by mPower
		if(isTablet(ClientCollection.getAppContext()) && getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT){
        	mView.setPadding(45, 100, 25, 0);
        }else if(isTablet(ClientCollection.getAppContext()) && getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE){
        	mView.setPadding(100, 90, 100, 0);        	
        }else{
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
    	ClientCollection.getInstance().getActivityLogger().logScrollAction(this, t - oldt);    
    }
    ...
    private void addGroupText(FormEntryCaption[] groups) {
        ...
        if (s.length() > 0) {
            ...
            int questionFontsize = ClientCollection.getInstance().getQuestionFontsize();
