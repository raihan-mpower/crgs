...
import com.mpower.clientcollection.database.ItemsetDbAdapter;
...
public class FormsProvider extends ContentProvider{
	...
	public synchronized Uri insert(Uri uri, ContentValues initialValues) {
		...
		if (values.containsKey(FormsColumns.JRCACHE_FILE_PATH) == false) {
			String cachePath = FormsDirectory.CACHE_PATH + File.separator + md5
					+ ".formdef";
				...
				if (update.getCount() > 0) {
					...
					if (values.containsKey(FormsColumns.FORM_FILE_PATH)) {
						...
						values.put(FormsColumns.JRCACHE_FILE_PATH,
								FormsDirectory.CACHE_PATH + File.separator + newMd5
										+ ".formdef");
					}
