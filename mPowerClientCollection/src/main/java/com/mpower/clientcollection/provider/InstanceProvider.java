...
package com.mpower.clientcollection.provider;
...
import com.mpower.clientcollection.database.ClientCollectionSQLiteOpenHelper;
...
public class InstanceProvider extends ContentProvider {
     ...
     */
    private static class DatabaseHelper extends ClientCollectionSQLiteOpenHelper {

        DatabaseHelper(String databaseName) {
            super(FormsDirectory.METADATA_PATH, databaseName, null, DATABASE_VERSION);
        }
        ...
        public void onCreate(SQLiteDatabase db) {
           db.execSQL("CREATE TABLE " + INSTANCES_TABLE_NAME + " ("
               ...
               + InstanceColumns.FILE_SIZE + " integer default 0, " // MPOWER
               + InstanceColumns.PRIORITY_VALUE + " integer default 0, " // MPOWER
               + InstanceColumns.SCHEDULE_ID + " text, "
               + InstanceColumns.LAST_STATUS_CHANGE_DATE + " date not null, "
               + InstanceColumns.DISPLAY_SUBTEXT + " text not null );");
           
           Log.d("InstanceProvider", "database for instance provider here");
        }
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if (rowId > 0) {
        	...
        	ClientCollection.getInstance().getActivityLogger().logActionParam(this, "insert",
    ...
    private void deleteAllFilesInDirectory(File directory) {
        if (directory.exists()) {
        	...
        	// ClientCollection Tables instance data directory. Let ClientCollection Tables
        	// manage the lifetimes of its filled-in form data
        	// media attachments.
            if (directory.isDirectory() && !ClientCollection.isClientCollectionTablesInstanceDataDirectory(directory)) {
                ...
                try {
                	...
                	if (del.getCount() > 0) {
                		...
                		do {
		                    ...
		                    ClientCollection.getInstance().getActivityLogger().logAction(this, "delete", instanceFile);
    ...
    static {
        ...
        sInstancesProjectionMap.put(InstanceColumns.FILE_SIZE, InstanceColumns.FILE_SIZE);
        sInstancesProjectionMap.put(InstanceColumns.SCHEDULE_ID, InstanceColumns.SCHEDULE_ID);
