...
public class ItemsetWidget extends QuestionWidget implements
    ...
    protected ItemsetWidget(Context context, FormEntryPrompt prompt,
        ...
        for (int i = 0; i < arguments.size(); i++) {
            ...
            if (xpr != null) {
                FormDef form = ClientCollection.getInstance().getFormController().getFormDef();
        ...
        if (nullArgs) {
            ...
            // TODO: put an error?
        } else if (ClientCollection.getInstance().getFormController().getItemsetHash() != null) {

            ItemsetDbAdapter ida = new ItemsetDbAdapter();
            ida.open();

            try {
                Cursor c = ida.query(ClientCollection.getInstance().getFormController()
                ...
                if (c != null) {
                    ...
                    while (c.moveToNext()) {
                        ...
                        String lang = ClientCollection.getInstance().getFormController().getLanguage();
            ...
            TextView error = new TextView(context);
            File csv = new File(ClientCollection.getInstance().getFormController().getMediaFolder()
                    + "/itemsets.csv");
