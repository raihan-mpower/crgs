 ...
 *
 * The logging database will be "/sdcard/mIntel/log/activityLog.db"
 *
 * Logging is enabled if the file "/sdcard/mIntel/log/enabled" exists.
 *
...
public final class ActivityLogger {
    ...
    private static class DatabaseHelper extends ClientCollectionSQLiteOpenHelper {
		...
		DatabaseHelper() {
			super(FormsDirectory.LOG_PATH, DATABASE_NAME, null, DATABASE_VERSION);
			new File(FormsDirectory.LOG_PATH).mkdirs();
		}
	...
	public ActivityLogger(String deviceId) {
		...
		mLoggingEnabled = new File(FormsDirectory.LOG_PATH, ENABLE_LOGGING).exists();
        open();
	}
    ...
    private String getXPath(FormIndex index) {
    	...
    	cachedXPathValue = ClientCollection.getInstance().getFormController().getXPath(index);
