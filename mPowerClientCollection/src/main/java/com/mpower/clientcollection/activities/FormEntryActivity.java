...
import com.mpower.clientcollection.listeners.AdvanceToNextListener;
import com.mpower.clientcollection.listeners.FormLoaderListener;
import com.mpower.clientcollection.listeners.FormSavedListener;
import com.mpower.clientcollection.logic.FormController;
import com.mpower.clientcollection.logic.FormController.FailedConstraint;
import com.mpower.clientcollection.logic.PropertyManager;
import com.mpower.clientcollection.preferences.AdminPreferencesActivity;
...
import com.mpower.clientcollection.tasks.FormLoaderTask;
import com.mpower.clientcollection.tasks.SaveToDiskTask;
...
public class FormEntryActivity extends Activity implements AnimationListener,
	...
	public static final int WEB_CAPTURE = 18;

	// Extra returned from gp activity
	public static final String LOCATION_RESULT = "LOCATION_RESULT";
	public static final String BEARING_RESULT = "BEARING_RESULT";
	public static final String WEB_RESULT = "WEB_RESULT";
	...
	//MPOWER: used to limit send form automatically when wifi or network found based on priority.
	private boolean mAutoSendPriority = false;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (data instanceof FormLoaderTask) {
			...
			if (!newForm) {
				if (ClientCollection.getInstance().getFormController() != null) {
					refreshCurrentView();
				} else {
			...
			ClientCollection.getInstance().setFormController(null);
			CompatibilityUtils.invalidateOptionsMenu(this);

			Intent intent = getIntent();
			if (intent != null) {
				Uri uri = intent.getData();
				LogUtils.debugLog(this, "uri = " + uri);
				...
				if (getContentResolver().getType(uri) == InstanceColumns.CONTENT_ITEM_TYPE) {
					...
					try {
						...
						if (c.getCount() != 1) {
							...
							File cacheDir = new File(FormsDirectory.CACHE_PATH);
							...
							for (int i = 0; i < files.length; ++i) {
								...
								File instanceDir = new File(
										FormsDirectory.INSTANCES_PATH + File.separator
												+ instanceDirName);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		FormController formController = ClientCollection.getInstance()
				.getFormController();
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		if (resultCode == RESULT_CANCELED) {
			...
			if (requestCode != HIERARCHY_ACTIVITY) {
				((ClientCollectionView) mCurrentView).cancelWaitingForBinaryData();
			}
			return;
		}
		...
		case BARCODE_CAPTURE:
			...
			((ClientCollectionView) mCurrentView).setBinaryData(sb);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case EX_STRING_CAPTURE:
			String sv = intent.getStringExtra("value");
			((ClientCollectionView) mCurrentView).setBinaryData(sv);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case EX_INT_CAPTURE:
			Integer iv = intent.getIntExtra("value", 0);
			((ClientCollectionView) mCurrentView).setBinaryData(iv);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case EX_DECIMAL_CAPTURE:
			Double dv = intent.getDoubleExtra("value", 0.0);
			((ClientCollectionView) mCurrentView).setBinaryData(dv);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case DRAW_IMAGE:
		case ANNOTATE_IMAGE:
		...
		case IMAGE_CAPTURE:
			 ...
			 * be in: /sdcard/ClientCollection/instances/[current instnace]/something.jpg so
			...
			// file
			File fi = new File(FormsDirectory.TMPFILE_PATH);
			...
			((ClientCollectionView) mCurrentView).setBinaryData(nf);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case ALIGNED_IMAGE:
			/*
		...
		case IMAGE_CHOOSER:
			 ...
			 * /sdcard/ClientCollection/instances/[current instnace]/something.jpg so we move
			...
			((ClientCollectionView) mCurrentView).setBinaryData(newImage);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case AUDIO_CAPTURE:
		case VIDEO_CAPTURE:
		...
		case VIDEO_CHOOSER:
			...
			((ClientCollectionView) mCurrentView).setBinaryData(media);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case LOCATION_CAPTURE:
			String sl = intent.getStringExtra(LOCATION_RESULT);
			((ClientCollectionView) mCurrentView).setBinaryData(sl);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case WEB_CAPTURE:
			String val = intent.getStringExtra(WEB_RESULT);
			((ClientCollectionView) mCurrentView).setBinaryData(val);
			saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
			break;
		case BEARING_CAPTURE:
            String bearing = intent.getStringExtra(BEARING_RESULT);
            ((ClientCollectionView) mCurrentView).setBinaryData(bearing);
            saveAnswersForCurrentScreen(DO_NOT_EVALUATE_CONSTRAINTS);
		...
		case MENU_SAVE:
			ClientCollection.getInstance()
					...
					.logInstanceAction(this, "onOptionsItemSelected",
							"MENU_SAVE");
			// don't exit
			saveDataToDisk(DO_NOT_EXIT, isInstanceComplete(false), null, null);
			return true;
	...
	private boolean saveAnswersForCurrentScreen(boolean evaluateConstraints) {
		...
		// group
		if (formController.currentPromptIsQuestion()) {
			LinkedHashMap<FormIndex, IAnswerData> answers = ((ClientCollectionView) mCurrentView)
					.getAnswers();
	...
	public boolean onContextItemSelected(MenuItem item) {
		 ...
		 * clicked on.
		 */
		for (QuestionWidget qw : ((ClientCollectionView) mCurrentView).getWidgets()) {
	...
	private View createView(int event, boolean advancingPage) {
		...
		case FormEntryController.EVENT_END_OF_FORM:
			...
			// MPOWER: Spinner for current form priority setting
			final Spinner formPriority = (Spinner)endView.findViewById(R.id.priority_spinner);
			final TextView priorityText = (TextView)endView.findViewById(R.id.priority_text);
			
			SharedPreferences mGeneralPreference = PreferenceManager.getDefaultSharedPreferences(ClientCollection.getAppContext());
			String autoSendWifiCategory = mGeneralPreference.getString(PreferencesActivity.KEY_AUTOSEND_WIFI, PreferencesActivity.KEY_AUTOSEND_WIFI_DEFAULT);
			String autSendNetworkCategory = mGeneralPreference.getString(PreferencesActivity.KEY_AUTOSEND_NETWORK, PreferencesActivity.KEY_AUTOSEND_NETWORK_DEFAULT);
			
			if(autoSendWifiCategory.equalsIgnoreCase(PreferencesActivity.KEY_AUTOSEND_BY_PRIORITY) 
					|| autSendNetworkCategory.equalsIgnoreCase(PreferencesActivity.KEY_AUTOSEND_BY_PRIORITY)){
				if(!formPriority.isShown() || !priorityText.isShown()){
					//active priority setting spinner, when user want to send via priority setting
					formPriority.setVisibility(View.VISIBLE);
					priorityText.setVisibility(View.VISIBLE);
				}				
				mAutoSendPriority = true;
			}else{
				//disable spinner 
				formPriority.setVisibility(View.GONE);
				//disable text
				priorityText.setVisibility(View.VISIBLE);
				mAutoSendPriority = false;
			}
					...
					.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							if (saveAs.getText().length() < 1) {
								...
								if(mAutoSendPriority)
								{
									// MPOWER: Spinner options position represent priority value. Here, 0 means high priority 
									String priorityValue = String.valueOf(formPriority.getSelectedItemPosition());
									saveDataToDisk(EXIT, instanceComplete
										.isChecked(), saveAs.getText()
										.toString(), priorityValue);
									Log.d("FormentryActivity", "priorityValue = " + priorityValue);
								}else{
									saveDataToDisk(EXIT, instanceComplete
											.isChecked(), saveAs.getText()
											.toString(),null);
		...
		case FormEntryController.EVENT_REPEAT:
			ClientCollectionView clientCollectionv = null;
			// should only be a group here if the event_group is a field-list
			try {
				FormEntryPrompt[] prompts = formController.getQuestionPrompts();
				FormEntryCaption[] groups = formController
						.getGroupsForCurrentIndex();
				clientCollectionv = new ClientCollectionView(this, formController.getQuestionPrompts(),
			...
			for (QuestionWidget qw : clientCollectionv.getWidgets()) {
	 ...
	 */
	// MPOWER: PriorityValuse is added for sending form based on priority value. 
	private boolean saveDataToDisk(boolean exit, boolean complete,
			String updatedSaveName, String priorityValue) {
		// save current answer
		...
		mSaveToDiskTask = new SaveToDiskTask(getIntent().getData(), exit,
				complete, updatedSaveName, priorityValue);
	...
	private void removeTempInstance() {
		...
		boolean erase = false;
		{
			Cursor c = null;
			try {
				Log.d("FormEntryActivity",InstanceColumns.CONTENT_URI.toString());
				...
				.setSingleChoiceItems(languages, selected,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int whichButton) {
								FormController formController = ClientCollection
	...
	protected void onResume() {
		...
		FormController formController = ClientCollection.getInstance()
				.getFormController();
		ClientCollection.getInstance().getActivityLogger().open();
	...
	private void afterAllAnimations() {
		...
		if (mStaleView != null) {
			if (mStaleView instanceof ClientCollectionView) {
				// http://code.google.com/p/android/issues/detail?id=8488
				((ClientCollectionView) mStaleView).recycleDrawables();
			}
			mStaleView = null;
		}

		if (mCurrentView instanceof ClientCollectionView) {
			((ClientCollectionView) mCurrentView).setFocus(this);
		}
		mBeenSwiped = false;
	}
	...
	public void loadingComplete(FormLoaderTask task) {
		...
		t.cancel(true);
		t.destroy();
		ClientCollection.getInstance().setFormController(formController);
		CompatibilityUtils.invalidateOptionsMenu(this);
		...
		// things then
		// returned to ClientCollection Collect and chose Edit Saved Form, but that the
		// savepoint for that
		...
		if (formController.getInstancePath() == null) {
			...
			String file = mFormPath.substring(mFormPath.lastIndexOf('/') + 1,
					mFormPath.lastIndexOf('.'));
			String path = FormsDirectory.INSTANCES_PATH + File.separator + file + "_"
					+ time;
			...
			boolean showEnd = reqIntent.getBooleanExtra("end", false);

			if(showEnd){
				ClientCollection.getInstance().getActivityLogger().logInstanceAction(this, "jumpToEnd", "click");
                ClientCollection.getInstance().getFormController().jumpToIndex(FormIndex.createEndOfFormIndex());
                setResult(RESULT_OK);
               // return;
			}else if (!showFirst) {
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		if (doSwipe) {
			...
			if (mCurrentView instanceof ClientCollectionView) {
				if (((ClientCollectionView) mCurrentView).suppressFlingGesture(e1, e2,
						velocityX, velocityY)) {
					return false;
				}
			}
	...
	private void sendSavedBroadcast() {
		Intent i = new Intent();
		i.setAction("com.mpower.clientcollection.FormSaved");
