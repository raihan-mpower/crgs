...
import com.mpower.clientcollection.utilities.DocumentFetchResult;
...
public class DownloadFormListTask extends AsyncTask<Void, String, HashMap<String, FormDetails>> {
    ...
    protected HashMap<String, FormDetails> doInBackground(Void... values) {
        SharedPreferences settings =
            PreferenceManager.getDefaultSharedPreferences(ClientCollection.getInstance().getBaseContext());
        ...
        String formListUrl = ClientCollection.getInstance().getApplicationContext().getString(R.string.default_clientcollection_formlist);
        String downloadPath = settings.getString(PreferencesActivity.KEY_FORMLIST_URL, formListUrl);
        downloadListUrl += downloadPath;

    	ClientCollection.getInstance().getActivityLogger().logAction(this, formListUrl, downloadListUrl);
        ...
        if (result.isOpenRosaResponse) {
            ...
            if (!xformsElement.getName().equals("xforms")) {
                ...
                formList.put(
                    DL_ERROR_MSG,
                    new FormDetails(ClientCollection.getInstance().getString(
            ...
            for (int i = 0; i < nElements; ++i) {
                ...
                for (int j = 0; j < fieldCount; ++j) {
                    ...
                    } else if (tag.equals("mMedia")) {
                        formName = XFormParser.getXMLText(child, true);
                        if (formName != null && formName.length() == 0) {
                            formName = null;
                        }
                    }else if (tag.equals("mediaURLs")) {
                        formName = XFormParser.getXMLText(child, true);
                        if (formName != null && formName.length() == 0) {
                            formName = null;
                        }
                ...
                formList.put(formId, new FormDetails(formName, downloadUrl, manifestUrl, formId, (version != null) ? version : majorMinorVersion, mMedia, mediaURLs));
            }
        } else {
            ...
            for (int i = 0; i < formsCount; ++i) {
                ...
                if (tag.equalsIgnoreCase("form")) {
                    ...
                    String isMedia = child.getAttributeValue(null, "isMedia");
                    String mediaURLs = child.getAttributeValue(null, "mediaURLs");
                    ...
                    formList.put(formName, new FormDetails(formName, downloadUrl, null, formId, null, isMedia, mediaURLs));
