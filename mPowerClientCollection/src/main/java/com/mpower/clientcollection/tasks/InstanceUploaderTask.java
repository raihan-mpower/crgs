...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, InstanceUploaderTask.Outcome> {
    ...
    private boolean uploadOneSubmission(String urlString, String id, String instanceFilePath,
    			Uri toUpdate, HttpContext localContext, Map<Uri, Uri> uriRemap, Outcome outcome, String scheduleId) {

    	ClientCollection.getInstance().getActivityLogger().logAction(this, urlString, instanceFilePath);
        ...
        if (uriRemap.containsKey(u)) {
            ...
            try {
                ...
                ClientCollection.getInstance().getContentResolver().update(toUpdate, cv, null, null);
                return true;
        ...
        while (j < files.size() || first) {
			...
			//edited from mPower. scheduleId and form checked status are added in here
			try {
				
				if(scheduleId != null){
					StringBody sb = new StringBody(scheduleId);
					entity.addPart("scheduleId", sb);
					Log.d("InstanceUploadTask", "scheduleId = " + scheduleId);
				}
            ...
            try {
                ...
                postResponse = WebUtils.getHttpPostResponse(response);
                WebUtils.discardEntityBytes(response);          
             
                Log.i(t, "Response code:" + responseCode + " Received response:" + postResponse);
                ...
                if (responseCode != HttpStatus.SC_CREATED && responseCode != HttpStatus.SC_ACCEPTED) {
                    if (responseCode == HttpStatus.SC_OK) {
                    	...
                    	if(postResponse != null && postResponse.length() > 0){
                    		   outcome.mResults.put(id, postResponse);
                    	}else{
                    		outcome.mResults.put(id, fail + response.getStatusLine().getReasonPhrase()
                                    + " (" + responseCode + ") at " + urlString);
                    	}
        ...
        // MPOWER:
        if(postResponse != null && postResponse.length() > 0){
        	// set server response if present
 		   outcome.mResults.put(id, postResponse);
        }else{
        	// There is no response from server, use default string
        	outcome.mResults.put(id, ClientCollection.getInstance().getString(R.string.success));
        }
    ...
    protected Outcome doInBackground(Long... values) {
        ...
        String deviceId = new PropertyManager(ClientCollection.getInstance().getApplicationContext())
        ...
        try {
	        ...
	        if (c.getCount() > 0) {
	            ...
	            while (c.moveToNext()) {
	                ...
	                String scheduleId = c.getString(c.getColumnIndex(InstanceColumns.SCHEDULE_ID));
	                ...
	                if (urlString == null) {
	                    SharedPreferences settings =
	                        PreferenceManager.getDefaultSharedPreferences(ClientCollection.getInstance());
	                    urlString = settings.getString(PreferencesActivity.KEY_SERVER_URL,
	                    				ClientCollection.getInstance().getString(R.string.default_server_url));
	                    ...
	                    String submissionUrl =
	                        settings.getString(PreferencesActivity.KEY_SUBMISSION_URL,
	                        		ClientCollection.getInstance().getString(R.string.default_clientcollection_submission));
	                ...
	                Log.d(InstanceUploaderTask.class.getSimpleName(), "submisionUrl = " + urlString);
	                if ( !uploadOneSubmission(urlString, id, instance, toUpdate, localContext, uriRemap, outcome, scheduleId) ) {
