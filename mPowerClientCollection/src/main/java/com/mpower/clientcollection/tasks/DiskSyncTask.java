...
public class DiskSyncTask extends AsyncTask<Void, String, String> {
    ...
    protected String doInBackground(Void... params) {
    	...
    	try {
	        ...
	        File formDir = new File(FormsDirectory.FORMS_PATH);
	        if (formDir.exists() && formDir.isDirectory()) {
	            // Get all the files in the /ClientCollection/foms directory
		        ...
		        try {
		            mCursor = ClientCollection.getInstance().getContentResolver()
	        ...
	        if ( errors.length() != 0 ) {
	        	...
	        	statusMessage = ClientCollection.getInstance().getString(R.string.finished_disk_scan);
	        }
    ...
    public ContentValues buildContentValues(File formDefFile) throws IllegalArgumentException {
        ...
        if (title != null) {
        	...
        	throw new IllegalArgumentException(ClientCollection.getInstance().getString(R.string.xform_parse_error,
